...
from src.gather_data.NaverCafeBlogArticleCounts import NaverCafeBlogArticleCounts

class FormOutputData:
    def __init__(self, selenium_object: SeleniumSettings) -> None:
        self.selenium_object = selenium_object 

    def form_output_settings_method(self, status_merge_col: str, output_file_name: str, output_sheets_names: list[str], naver_keywords_settings_dict: dict, naver_top_keywords_settings_dict: dict, naver_views_settings_dict: dict, naver_articles_settings_dict: dict, status_rename_dict: dict, keywords_rename_dict: dict, keywords_status_rename_dict: dict, top_keywords_rename_dict: dict, top_keywords_status_rename_dict: dict, views_rename_dict: dict, views_status_rename_dict: dict, articles_rename_dict: dict, articles_status_rename_dict: dict) -> None:
        ...
        self.naver_articles_settings_dict     = naver_articles_settings_dict
        ...
        self.articles_rename_dict             = articles_rename_dict 
        self.articles_status_rename_dict      = articles_status_rename_dict 
    ...
    def naver_article_counts_method(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        self.naver_cafe_blog_object = NaverCafeBlogArticleCounts(self.selenium_object)
        self.naver_cafe_blog_object.naver_article_counts_settings_method(**self.naver_articles_settings_dict)
        self.naver_cafe_blog_object.get_cafe_article_counts()
        self.naver_cafe_blog_object.get_blog_article_counts()

        naver_articles_status_data = pd.DataFrame(self.naver_cafe_blog_object.naver_article_counts_status_check).drop(columns = ["date"]).rename(columns = self.articles_status_rename_dict)
        naver_articles_dataframe   = pd.DataFrame(self.naver_cafe_blog_object.naver_article_counts_dictionary).rename(columns = self.articles_rename_dict)

        return naver_articles_status_data, naver_articles_dataframe
    ...
    def finalize_outputs(self) -> tuple[pd.ExcelWriter, list[pd.DataFrame]]:
        ...
        naver_articles_status_data, naver_articles_dataframe    = self.naver_article_counts_method()
        ...
        final_status_output = pd.merge(output_status_data, naver_articles_status_data, on = self.status_merge_col, how = "left").rename(columns = self.status_rename_dict)
        excel_writer_object = pd.ExcelWriter(self.output_file_name.format(str(dt.datetime.now())[0:10]), engine = "openpyxl")
        results_data_list   = [final_status_output, naver_data_dataframe, naver_top_keywords_data, naver_views_dataframe, naver_articles_dataframe]
